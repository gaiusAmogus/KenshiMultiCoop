...
REM Locate MSBuild via vswhere. Use -products * so it also finds VS Build Tools
REM and older installs (VS2017), not just a "-latest" full VS - "-latest" alone
REM returns nothing on a machine whose only install is 2017 BuildTools.
set "VSWHERE=%ProgramFiles(x86)%\Microsoft Visual Studio\Installer\vswhere.exe"
set "MSBUILD="
for /f "usebackq delims=" %%i in (`"%VSWHERE%" -products * -requires Microsoft.Component.MSBuild -find "MSBuild\**\Bin\MSBuild.exe" 2^>nul`) do if not defined MSBUILD set "MSBUILD=%%i"
REM Fallbacks to known install paths if vswhere returned nothing (nested-quote
REM edge cases under some shells). Try VS2022, then VS2019, then VS2017 BuildTools.
...
if not defined MSBUILD if exist "C:\Program Files (x86)\Microsoft Visual Studio\2017\BuildTools\MSBuild\15.0\Bin\MSBuild.exe" set "MSBUILD=C:\Program Files (x86)\Microsoft Visual Studio\2017\BuildTools\MSBuild\15.0\Bin\MSBuild.exe"
...
REM NOTE: do NOT add %KL%\KenshiLib\Include\kenshi here. KenshiLib headers use
REM #pragma once, which keys on file PATH; exposing kenshi\ as a second root lets
REM the same header be reached two ways (<kenshi/Building/Building.h> and
REM <Building/Building.h>), defeating the guard and causing enum redefinition
REM (BuildingDesignation). The CombatClass 0.4.0 relocation is handled by the
REM vc10_compat\kenshi\CombatClass.h shim instead (which pre-includes the
REM siblings by full <kenshi/...> path before pulling the combat\ header).
