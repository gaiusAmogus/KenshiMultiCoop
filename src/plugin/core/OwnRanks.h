...
// Resolve the ownership ranks for a network player id. rank == playerId:
// host is playerId 0 (owns tab {0}), join#1 is playerId 1 (owns {1}),
// join#2 is playerId 2 (owns {2}), etc. This is the N-player generalization
// of the old host/join binary default.
//   fromEnv == true : ranks came from an explicit env override - preserve them.
//   fromEnv == false: use the default {playerId}.
// Safe to call repeatedly; on a role/id change the default is recomputed.
inline void resolveOwnRanksForPlayer(std::set<unsigned int>& ranks,
                                     unsigned int playerId, bool fromEnv) {
    if (fromEnv) return;
    ranks.clear();
    ranks.insert(playerId);
}

// Back-compat wrapper: the pre-3-player call sites pass a bool role. Host maps
// to playerId 0, any non-host to playerId 1 (the historical 2-player default).
// New code paths that know the real network id should call
// resolveOwnRanksForPlayer directly (see Plugin.cpp connect edge).
inline void resolveOwnRanks(std::set<unsigned int>& ranks, bool isHost, bool fromEnv) {
    resolveOwnRanksForPlayer(ranks, isHost ? 0u : 1u, fromEnv);
}
