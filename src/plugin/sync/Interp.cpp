...
bool EntityInterp::sample(unsigned long nowMs, const InterpConfig& cfg, EntityState* out) {
    ...
    if (count_ >= 2) {
        ...
        // Cap raised 6000 -> 8000 ms: with more players the mid-band round-robin
        // has more candidates sharing the quota, so the gap between an NPC's
        // samples grows and a 6 s cap flapped crowd NPCs out of the driven set on
        // a slow rotation. 8 s still releases a genuinely abandoned body within a
        // couple seconds. Harmless for 2 players (seg4 rarely reaches 6 s there).
        if (stale > 8000) stale = 8000;
    }
