...
void Replicator::publishOwned(GameWorld* gw, NetLink& net, u32 ownerId) {
    ...
    if (streamNpcs_ && !midBand_.empty() && n < MAX_PUBLISH) {
        ...
        // Scale the mid-band quota with the number of connected players. With more
        // players there are more squads in motion and thus more nearby NPCs
        // competing for the same fixed ~2 Hz mid-band budget, so each NPC's
        // refresh rate collapses (visible as stutter/rubber-banding in a crowd).
        // Multiply the quota (and its cap) by the peer count so aggregate refresh
        // holds. activePeerCount_ is 1 in the 2-player case, so quota/cap are
        // byte-identical to the pre-3-player behaviour (no regression). Capped so
        // a large lobby can't blow the host's send budget unbounded.
        if (activePeerCount_ > 1u) {
            quota *= activePeerCount_;
            unsigned int cap = 16u * activePeerCount_;
            if (cap > 48u) cap = 48u;      // hard ceiling (was 16 for 2 players)
            if (quota > cap) quota = cap;
        }
    ...
    // Diagnostic (throttled ~1 Hz): if a tick fills the whole MAX_PUBLISH buffer
    // we are at the publish ceiling and further nearby entities were silently
    // dropped this tick - the crowd-scaling signal to watch in a 3-player run.
    // Pure logging; no runtime behaviour change.
    if (n >= MAX_PUBLISH) {
        unsigned long nowCap = nowMs();
        if (capLogMs_ == 0 || (nowCap - capLogMs_) >= 1000) {
            capLogMs_ = nowCap;
            char cb[112]; _snprintf(cb, sizeof(cb) - 1,
                "[publish] AT CAP n=%u max=%u peers=%u (nearby entities dropped)",
                n, (unsigned)MAX_PUBLISH, activePeerCount_);
