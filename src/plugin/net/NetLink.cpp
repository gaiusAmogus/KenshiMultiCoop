...
// 3-player relay whitelist: which received packet types the HOST may forward
// verbatim to the OTHER joins (so join#1 and join#2 see each other). ONLY
// owner-authoritative BROADCAST state belongs here - packets whose owner is the
// sole authority and which the host would rebroadcast anyway. Request/intent
// packets that the HOST arbitrates (speed, spawn resolve, save/load
// coordination, conservation drop/pickup/transfer) are deliberately EXCLUDED:
// relaying them raw would let a join receive an unarbitrated request as if it
// were authoritative, racing the host's processed result. This foundation phase
// whitelists only presence/movement; extend per channel as each channel's
// 3-player arbitration is designed and validated.
bool relayableToPeers(u8 type) {
    switch (type) {
        case PKT_ENTITY_BATCH: // 20 Hz squad/NPC transforms (owner-authoritative)
        case PKT_EVENT:        // one-shot transitions (KO/death/revive), owner-authored
            return true;
        default:
            return false;
    }
...
void NetLink::threadLoop() {
    ...
    u32   nextId = 1;
    // Stable playerId assignment (host): map a peer's network address -> the id
    // it was first given, so a RECONNECT of the same physical player (flaky
    // Steam/UDP link, client relaunch while the host keeps running, ENet's own
    // reconnect) reuses its original id instead of minting a fresh one via
    // nextId++. playerId == squad-tab rank, so without this a mid-session
    // reconnect silently moved a player from tab 1 to tab 2 (they then drove a
    // tab they no longer streamed, and the host saw only the other tab move).
    // Keyed by address.host (IP for UDP; the stable per-peer virtual address for
    // the Steam tunnel). Lives for the host thread's lifetime, like nextId.
    std::map<enet_uint32, u32> idByAddr;
        ...
        while (enet_host_service(enetHost_, &ev, TICK_MS) > 0) {
            switch (ev.type) {
                ...
                case ENET_EVENT_TYPE_RECEIVE: {
                    ...
                    // 3-player relay (host only): so join#1 and join#2 can see each
                    // other, the host forwards a peer's OWNER-AUTHORITATIVE broadcast
                    // state to the OTHER joins (star topology: join#1 -> host ->
                    // join#2). The packet already carries ownerId, so receivers
                    // attribute it correctly and we can forward the exact bytes.
                    //
                    // IMPORTANT: only WHITELISTED broadcast-state packets are relayed.
                    // We must NOT blanket-forward every type: many packets are
                    // REQUESTS the host is the ARBITER of (speed, spawn resolve, save/
                    // load coordination, conservation drop/pickup/transfer). Relaying
                    // those raw would hand join#2 an unarbitrated request as if it were
                    // authoritative, racing the host's own processed result -> desync/
                    // double-apply. Those types flow ONLY through host processing ->
                    // host broadcast, never this relay. This foundation phase relays
                    // just presence/movement state; later phases extend the whitelist
                    // per channel once each channel's 3-player arbitration is designed.
                    if (isHost_ && relayableToPeers(type)) {
                        for (size_t pi = 0; pi < enetHost_->peerCount; ++pi) {
                            ENetPeer* dst = &enetHost_->peers[pi];
                            if (dst->state != ENET_PEER_STATE_CONNECTED) continue;
                            if (dst == ev.peer) continue; // never echo to sender
                            ENetPacket* fwd = enet_packet_create(
                                ev.packet->data, ev.packet->dataLength,
                                ev.packet->flags);
                            enet_peer_send(dst, ev.channelID, fwd);
                        }
                    }
                    if (isHost_ && type == PKT_HELLO) {
                        ...
                        if (readPacket(ev.packet->data, (unsigned)ev.packet->dataLength, &h)) {
                            if (h.version != PROTOCOL_VERSION) {
                                ...
                                // Stable playerId: reuse the id previously granted
                                // to this peer's address if it is reconnecting;
                                // otherwise mint a fresh one. This keeps a player's
                                // squad-tab rank stable across a mid-session
                                // reconnect (see idByAddr note above).
                                enet_uint32 addr = ev.peer->address.host;
                                std::map<enet_uint32, u32>::iterator ait =
                                    idByAddr.find(addr);
                                u32 id;
                                bool returning = (ait != idByAddr.end());
                                if (returning) {
                                    id = ait->second;
                                } else {
                                    id = nextId++;
                                ...
                                // 3+ players are supported via host raw-relay (see the
                                // relay block in ENET_EVENT_TYPE_RECEIVE): the host
                                // forwards join-authored packets to the other joins.
                                {
                                    char nb[120];
                                    _snprintf(nb, sizeof(nb) - 1,
                                        "peer id=%u %s (rank=%u)",
                                        (unsigned)id,
                                        returning ? "reconnected (stable id)"
                                                  : "connecting",
                                        (unsigned)id);
                                    nb[sizeof(nb) - 1] = '\0';
                                    netLog(nb);
