...
// Fired when the host clicks "Synchronize": the plugin root force-resends the
// host's world state to all joins (repairs a mid-session desync). The panel has
// no access to the Replicator/NetLink, so it routes the request up here, exactly
// like connect/disconnect.
typedef void (*CoopSyncFn)();
